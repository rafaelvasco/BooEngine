...
public static unsafe class BooBlitter
{
    private const int SDL_AUX_BUFFER_SIZE = 1024 * 1024 * 4;
    private const int SDL_AUX_BUFFER_STACK_SIZE = 2;
    ...
    private static readonly byte[]?[] AuxBuffers = new byte[SDL_AUX_BUFFER_STACK_SIZE][];
    ...
    public static void Begin(Memory<byte> pixels, int surfaceWidth, int surfaceHeight)
    {
        if (_ready)
        {
            throw new BooException("Cannot nest Begin call.");
        }
    ...
    {
        if (length > SDL_AUX_BUFFER_SIZE)
        {
            throw new BooException(
                $"Blitter GetCopy: Overflow. Length {length} is larger than max {SDL_AUX_BUFFER_SIZE}");
        }

        AuxBuffers[_auxBuffersIdx] ??= new byte[SDL_AUX_BUFFER_SIZE];
        ...
        if (_auxBuffersIdx > SDL_AUX_BUFFER_STACK_SIZE - 1)
        {
            _auxBuffersIdx = 0;
        }
