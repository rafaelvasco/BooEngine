...
public struct KeyboardState
{
    ...
    private static readonly Key[] PressedBuffer = new Key[Enum.GetValues(typeof(Key)).Length];
...
public class Keyboard
{
    ...
    internal Keyboard()
    {
        _kbState = BooPlatform.GetKeyboardState();
        _lastKbState = _kbState;

        BooPlatform.KeyDown = key => ProcessEvent(key, true);
        BooPlatform.KeyUp = key => ProcessEvent(key, false);
        BooPlatform.TextInput = ev => ProcessTextInput(ev);
    }
