...
public class Mouse
{
    ...
    internal Mouse()
    {
        _msState = BooPlatform.GetMouseState();

        BooPlatform.MouseDown = button => { OnMouseDown?.Invoke(button); };

        BooPlatform.MouseUp = button => { OnMouseUp?.Invoke(button); };

        BooPlatform.MouseMove = (x, y) => { OnMouseMove?.Invoke(x, y); };
    }
