...
internal static partial class BooPlatform
{
    ...
    private static readonly IntPtr[] _gamepadDevices = new IntPtr[Gamepad.MaxCount];
    ...
    {
        index = Calc.Clamp(index, 0, _gamepadDevices.Length - 1);
        
        return SDL_GameControllerName(_gamepadDevices[index]);
    }
    ...
    private static void AddGamePadInstance(int deviceId)
    {
        ...
        if (PlatformId == BooPlatformId.Linux &&
